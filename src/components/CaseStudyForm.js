...
function CaseStudyForm({ initialData = {}, onSave, onCancel }) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const data = {
        ...form,
        challenges: form.challenges.split(',').map(s => s.trim()).filter(Boolean),
        results: form.results.split(',').map(s => s.trim()).filter(Boolean),
        order: Number(form.order),
      };
      if (initialData.id) {
        await updateDoc(doc(db, 'caseStudies', initialData.id), data);
      } else {
        await addDoc(collection(db, 'caseStudies'), data);
      }
      setLoading(false);
      onSave && onSave();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{marginBottom:30,background:'#f9f9f9',padding:20,borderRadius:8}}>
      <h3>{initialData.id ? 'Edit' : 'Add'} Case Study</h3>
      <div style={{display:'flex',gap:20,flexWrap:'wrap'}}>
        <div style={{flex:1,minWidth:200}}>
          <label>Name<br/><input name="name" value={form.name} onChange={handleChange} required /></label>
        </div>
        <div style={{flex:1,minWidth:200}}>
          <label>Profession<br/><input name="profession" value={form.profession} onChange={handleChange} required /></label>
        </div>
        <div style={{flex:1,minWidth:120}}>
          <label>Duration<br/><input name="duration" value={form.duration} onChange={handleChange} required /></label>
        </div>
      ...
      <div style={{marginTop:15}}>
        <label>Image URL<br/>
          ...
          {form.img && <img src={form.img} alt="case study" style={{width:80,marginTop:5}} />}
          <input type="file" accept="image/*" onChange={handleFile} />
          {uploading && <span>Uploading...</span>}
        </label>
      </div>
      <div style={{marginTop:15}}>
        <label>Challenges (comma separated)<br/><input name="challenges" value={form.challenges} onChange={handleChange} /></label>
      </div>
      <div style={{marginTop:15}}>
        <label>Results (comma separated)<br/><input name="results" value={form.results} onChange={handleChange} /></label>
      </div>
